...
let adminStompClient: any = null;
let adminStompConnected = false;
let adminPendingSubscriptions: Array<() => void> = [];
...
export function useNotifications() {
    ...
    try {
      ...
      import('@stomp/stompjs').then(({ Client }) => {
        import('sockjs-client').then((SockJS) => {
          ...
          adminStompClient.onConnect = () => {
            console.log('Đã kết nối WebSocket cho Admin');
            adminStompConnected = true;

            // Đăng ký nhận thông báo
            subscribeToNotifications();

            // Thực hiện các đăng ký đang chờ
            adminPendingSubscriptions.forEach((subscribe) => subscribe());
            adminPendingSubscriptions = [];
          };

          adminStompClient.onStompError = (frame: any) => {
            console.error('Lỗi STOMP', frame);
            adminStompConnected = false;
  ...
  const subscribeToNotifications = useCallback(() => {
    ...
    const subscribeFunction = () => {
      if (!adminStompClient || !adminStompConnected) {
        adminPendingSubscriptions.push(subscribeFunction);
        return;
      }

      // Đăng ký nhận thông báo cá nhân
      adminStompClient.subscribe(`/user/queue/notifications`, (message: any) => {
        try {
      ...
      // Đăng ký nhận thông báo broadcast cho admin
      adminStompClient.subscribe('/topic/admin-notifications', (message: any) => {
        try {
          const newNotification = JSON.parse(message.body) as Notification;
          handleNewNotification(newNotification);
        } catch (err) {
          console.error('Lỗi xử lý thông báo broadcast cho admin:', err);
        }
      });
    };
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Không thể tải thông báo quản trị. Vui lòng thử lại sau.',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
  ...
  const clearAllNotifications = useCallback(async () => {
    ...
    try {
      const API_URL = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080/api';
      const response = await fetch(`${API_URL}/api/admin/notifications/clear-all`, {
        method: 'DELETE',
      });
  ...
  const archiveAllReadNotifications = useCallback(async () => {
    ...
    try {
      const API_URL = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080/api';
      const response = await fetch(`${API_URL}/api/admin/notifications/archive-all-read`, {
        method: 'POST',
      });
